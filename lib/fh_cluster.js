...
function startMaster( numWorkers ) {
  ...
  cluster.on('disconnect', setupOnExitBackoff());
...
function setupOnExitBackoff() {
  return function(worker) {
    ...
    console.log('Worker #', worker.id, 'disconnected. Will retry');
    backoff(function() {
      cluster.fork();
    });
  };
